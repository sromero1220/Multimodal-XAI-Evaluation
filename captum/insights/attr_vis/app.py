...
class AttributionVisualizer:
    ...
    ##########################################My code############################################
    def inspect_feature_outputs(self):
        for output, _ in self._outputs:
            for vis_output in output:
                for fo in vis_output.feature_outputs:
                    print(fo)
    def get_attributions(self):
        modality_attributions = []
        for output, _ in self._outputs:
            for vis_output in output:
                # Extracting the modified attribute which represents the attributions
                modality_feature_attributions = [fo.contribution for fo in vis_output.feature_outputs]
                modality_attributions.append(modality_feature_attributions)
        return modality_attributions
    ...
    def get_insights_config(self):
        return {
            ...
            "selected_arguments": self._config.attribution_arguments,
        }
